...
form.addEventListener('submit', e => {
    ...
    if (!namaTim || !timLead) {
        statusMessageContainer.style.color = 'red';
        statusMessageContainer.textContent = "Nama tim dan tim lead wajib diisi.";
        submitButton.disabled = false;
        submitButton.value = "Kirim";
        return;
    }
    ...
    fetch(sheetDataURL)
        ...
        .then(csvText => {
            ...
            if (isDuplicate) {
                statusMessageContainer.style.color = 'red';
                statusMessageContainer.textContent = "Data dengan nama tim dan tim lead ini sudah pernah dikirim.";
                submitButton.disabled = false;
                submitButton.value = "Kirim";
                return;
            }
    ...
    Promise.all(filePromises)
        ...
        .then(data => {
            if (data.result === 'success') {
                ...
                statusMessageContainer.style.color = 'red';
                statusMessageContainer.textContent = "Nama Tim sudah terdaftar, Anda tidak bisa mendaftar menggunakan nama tim ini.";
            } else {
